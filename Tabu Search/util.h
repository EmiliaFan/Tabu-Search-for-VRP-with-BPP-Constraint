...
namespace my_util {
	...
	bool comp_veh_free_area(const Vehicle &v1, const Vehicle &v2) {
		double free_area1 = v1.get_area() - v1.get_loaded_area();
		double free_area2 = v2.get_area() - v2.get_loaded_area();
		return free_area1 > free_area2;
	}

	bool comp_veh_distance(const Vehicle &v1, const Vehicle &v2) {
		double near1=999999, near2=999999;
		for (auto& sid : v1.visit_order)
		{
			if (distance_matrix.at(make_pair(move4_sid, sid)) < near1)
				near1 = distance_matrix.at(make_pair(move4_sid, sid));
		}
		for (auto& sid : v2.visit_order)
		{
			if (distance_matrix.at(make_pair(move4_sid, sid)) < near2)
				near2 = distance_matrix.at(make_pair(move4_sid, sid));
		}
		return near1 < near2;
	}
